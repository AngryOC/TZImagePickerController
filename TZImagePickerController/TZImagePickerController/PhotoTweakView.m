...
- (instancetype)initWithFrame:(CGRect)frame
{
    if (self = [super initWithFrame:frame]) {
        ...
        _upperLeft.center = CGPointMake(kCropViewCornerLength / 2 - halfLineWidth, kCropViewCornerLength / 2 - halfLineWidth);
        ...
        _upperRight.center = CGPointMake(self.frame.size.width - kCropViewCornerLength / 2 - halfLineWidth, kCropViewCornerLength / 2 + halfLineWidth);
        ...
        _lowerRight.center = CGPointMake(self.frame.size.width - kCropViewCornerLength / 2 + halfLineWidth, self.frame.size.height - kCropViewCornerLength / 2 + halfLineWidth);
        ...
        _lowerLeft.center = CGPointMake(kCropViewCornerLength / 2 - halfLineWidth, self.frame.size.height - kCropViewCornerLength / 2 + halfLineWidth);
...
//        CGFloat scale = MAX(scaleX, scaleY);
        ...
        //CGFloat scale = MAX(scaleX, scaleY);
        if (scaleX < scaleY) {
            _scrollView.contentSize = CGSizeMake(self.scrollView.bounds.size.width, image.size.height / scaleX);
            _scrollView.contentOffset = CGPointMake(0, (image.size.height / scaleX - self.scrollView.bounds.size.width)/2.0 );
        } else {
            _scrollView.contentSize = CGSizeMake(image.size.width / scaleY, self.scrollView.bounds.size.width);
            _scrollView.contentOffset = CGPointMake((image.size.width / scaleY - self.scrollView.bounds.size.width)/2.0,  0);
        }
        ...
        if (scaleX < scaleY) {
            _photoContentView.frame = CGRectMake(0, 0, self.scrollView.bounds.size.width, image.size.height / scaleX);//self.scrollView.bounds;
        } else {
            _photoContentView.frame = CGRectMake(0, 0, image.size.width / scaleY, self.scrollView.bounds.size.width);//self.scrollView.bounds;
        }
...
- (void)resetOriginImage:(UIImage *)image{
    _image = image;
    //_imageView.image = self.image;
    [_photoContentView resetOriginImage:image];
}
